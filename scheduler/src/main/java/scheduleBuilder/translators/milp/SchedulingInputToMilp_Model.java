...
public class SchedulingInputToMilp_Model implements MilpTranslator {
    ...
    private MilpModel buildModel() {
        ...
        addTeacherTimeWindowTimeslotConstraints();
    ...
    private void addTeacherTimeWindowConstraints() {
        addTeacherTimeWindowTimeslotConstraints();
        addTeacherTimeWindowDayConstraints();
        //addTeacherTimeWindowBetweenOccupiedSlotsConstraints();
    }

    private void addTeacherTimeWindowDayConstraints() {
        for (Teacher teacher : _teachers) {
            String teacherId = "" + _teachers.indexOf(teacher);
            for(var entry : _dayOfWeekTimeSlots.entrySet()) {
                DayOfWeek dayOfWeek = entry.getKey();
                ...
                Stream<String> timeWindowsVarLabels = timeSlots
                        .stream()
                        .filter(getTeacherTimeslotsPredicate(teacher))
                        .map(timeSlot -> List.of(teacherId, _timeSlotToIdMap.get(timeSlot)))
                        .map(list -> _nameSpace.getVariableName(_teacherTimeWindowVar, list));
                var constraint = _constraintFactory.createBinaryBlockingConstraint(getNextConstraintLabel(), teacherDayOfWeekVarLabel, timeWindowsVarLabels);
                _milpModelBuilder.addConstraint(constraint);
            }
        }
    }
    ...
    private void addTeacherTimeWindowTimeslotConstraints() {
        for (Teacher teacher : _teachers) {
            String teacherId = "" + _teachers.indexOf(teacher);
            for(var timeSlot : _timeSlotSequence) {
                String tsName =  _timeSlotToIdMap.get(timeSlot);
                // w + ts <= 1
                String windowVarName = _nameSpace.getVariableName(_teacherTimeWindowVar, List.of(teacherId, tsName));
                String timeSlotVarName = _nameSpace.getVariableName(_teacherTimeSlotVar, List.of(teacherId, tsName));

                Stream<Pair<String, Double>> pairStream = Stream.of(windowVarName, timeSlotVarName)
